...
class FinalHarness:
    ...
    # 각 규칙을 개별 on/off. 기본값은 모두 True (전체 채택 상태).
    # 일반화 코어: R1~R10 전부 False (독립순도 29~67%, Private 리스크).
    # 독립 predicate 테이블로 리팩토링돼 있어 개별 True로 켜서 A/B 실험 가능.
    RULES: dict[str, bool] = {
    ...
    def _control_fallback(self, records: list[dict], types: set[str], focal: dict, task: dict, session: dict, evidence: dict) -> str:
        R = self.RULES
        def rec_val(rt: str) -> str:
            return next((str(r.get("value", "")) for r in records if str(r.get("type")) == rt), "")

        esp  = rec_val("external_share_policy")
        sbu  = rec_val("share_boundary_update")
        dac  = rec_val("dispatch_authority_check")
        glad = "guardrail_ladder_signal" in types
        contains = set(str(x) for x in (focal.get("attrs") or {}).get("contains", []) or [])
        sens = bool(contains & self._SENSITIVE_CONTAINS)

        # 독립 predicate 테이블: (토글, 조건, 답, 우선순위).
        # 조건 상호배타 — dev+screening 820개에서 중첩 0건 확인.
        # 우선순위(작을수록 강): hold=0, amend=1, ask=2, proceed=3.
        # ⚠️ R1~R10 독립순도 29~67%(체인 착시). RULES 기본 False = 일반화 코어.
        table = [
            (R["R1"], esp == "doctor_note_forbidden",                                                   "hold",   0),
            (R["R2"], ("forbidden" in esp or "summary_only" in esp) and esp != "doctor_note_forbidden", "amend",  1),
            (R["R3"], sbu == "dispatch_blocked_until_binding" and dac == "user_binding_pending",        "hold",   0),
            (R["R4"], sbu == "dispatch_blocked_until_binding" and dac != "user_binding_pending",        "ask",    2),
            (R["R5"], sens and sbu == "redacted_external_boundary",                                     "amend",  1),
            (R["R6"], sens and sbu == "local_update_boundary",                                          "proceed",3),
            (R["R8"], not sens and sbu == "local_update_boundary" and not glad,                         "amend",  1),
            (R["R9"], not sens and sbu == "redacted_external_boundary" and not glad,                    "ask",    2),
            (R["R10"],not sens and sbu == "local_update_boundary" and glad,                             "ask",    2),
        ]
        fired = [(prio, ans) for cond, pred, ans, prio in table if cond and pred]
        if fired:
            return min(fired, key=lambda x: x[0])[1]
