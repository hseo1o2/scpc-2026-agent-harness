...
class FinalHarness:
    fallback_mode: str = "A"  # "A" = 보수적, "B" = 적극적
    ...
    def update_session_memory(self, task: dict[str, Any], session: dict[str, Any], evidence: dict[str, Any]) -> None:
        ...
        # ── 세션 상태: "이 turn에 확정된 신뢰 가능한 사실"만 축적 ──────────────
        # 원칙: 우리 예측값이 아닌 records가 명시한 확정값만 저장(오류 전파 차단).
        # 빈값 turn은 상태를 덮어쓰지 않고 보존 → 항상 "가장 최근 확정값" 유지.
        rm = {str(r.get("type")): r for r in records_of(task)}

        # (a) resolved_target record의 실제 값
        rt_rec = rm.get("resolved_target")
        if isinstance(rt_rec, dict):
            rt_val = rt_rec.get("value")
            if isinstance(rt_val, str) and rt_val.strip():
                session["last_resolved_target"] = rt_val.strip()

        # (b) target_changed_after_turn: 명시적 변경(짧은 slug만 신뢰)
        tc_rec = rm.get("target_changed_after_turn")
        if isinstance(tc_rec, dict):
            tc_val = tc_rec.get("value")
            if isinstance(tc_val, str) and tc_val.strip() and len(tc_val.split("_")) <= 3:
                session["last_resolved_target"] = tc_val.strip()
    ...
    def choose_focal(self, task: dict[str, Any], session: dict[str, Any], evidence: dict[str, Any]) -> dict[str, Any]:
        ...
        2) visible_history WM코드 방향성 키워드 근접도 (오른쪽 25자 우선)
        3) 베이스라인 fallback (record값→id, ref_code, prompt 토큰)
        """
    ...
    def infer_target(self, task: dict[str, Any], focal: dict[str, Any], control: str, session: dict[str, Any], evidence: dict[str, Any]) -> str:
        ...
        if control == "proceed":
            ...
            if sbu_val == "local_update_boundary" or dan_is_local:
                return "memory_store"
        ...
        # 4.5 세션 이어받기: resolved_target 비어있고 ambiguous_target 신호면
        #     세션에 축적된 "가장 최근 확정 target"을 이어받는다.
        #     근거: screening 4턴 세션 최빈 (확정,빈값,확정,빈값) 99개.
        #     turn4는 turn1이 아닌 turn3(최근 확정)을 이어받는 불변식.
        rt_empty = not (isinstance(rt_rec, dict)
                        and isinstance(rt_rec.get("value"), str)
                        and rt_rec.get("value").strip())
        amb_target = rec_val("ambiguous_target")
        if rt_empty and amb_target:
            carried = session.get("last_resolved_target")
            if carried:
                return carried
        ...
        return str(session.get("last_resolved_target") or "user")

    # ── control fallback (R1-R10 토글 실험용) ────────────────────────────────
    # B급 포함 전체 채택 상태. 개별 False로 ablation 가능.
    # 순수코어: 전부 False (독립순도 29~67%). public 실측 +0.0425 기여 확인.
    RULES: dict[str, bool] = {
        "R1": True, "R2": True, "R3": True, "R4": True, "R5": True,
        "R6": True, "R7": True, "R8": True, "R9": True, "R10": True,
    }
    ...
    def _control_fallback(self, records: list[dict], types: set[str], focal: dict, task: dict, session: dict, evidence: dict) -> str:
        ...
        # ── R1/R2: external_share_policy ─────────────────────────────────────
        esp_rec = next((r for r in records if r.get("type") == "external_share_policy"), None)
        if esp_rec:
            esp_val = str(esp_rec.get("value", ""))
            if R["R1"] and esp_val == "doctor_note_forbidden":
                return "hold"
            if R["R2"] and ("forbidden" in esp_val or "summary_only" in esp_val):
                return "amend"

        # ── R3/R4: dispatch_blocked_until_binding ─────────────────────────────
        ...
        if sbu_val == "dispatch_blocked_until_binding":
            if R["R3"] and dac_val == "user_binding_pending":
                return "hold"
            if R["R4"]:
                return "ask"

        # ── R5/R6: focal 민감 콘텐츠 + sbu ───────────────────────────────────
        attrs    = focal.get("attrs") or {}
        contains = set(str(x) for x in attrs.get("contains", []) or [])
        has_sensitive = bool(contains & self._SENSITIVE_CONTAINS)

        if has_sensitive:
            if R["R5"] and sbu_val == "redacted_external_boundary":
                return "amend"
            if R["R6"] and sbu_val == "local_update_boundary":
                return "proceed"

        # ── R8-R10: sbu + glad 조합 ───────────────────────────────────────────
        if sbu_val == "redacted_external_boundary":
            if R["R9"] and not glad:
                return "ask"

        if sbu_val == "local_update_boundary":
            if R["R8"] and not glad:
                return "amend"
            if R["R10"] and glad:
                return "ask"

        # ── ssp 규칙: session_share_policy=strict + 외부 전달 → amend ─────────
        # dev 실측 순도 16/16(출처 16세션). screening +73건, public 기여 확인.
        # normal에 적용 금지(4건 파손 확인). ROUTE_CLUSTER 보유 시 제외.
        ssp_strict = any(
    ...
    # ── visible_history 판단서술 분류기 (dev 42/42=100%, LODO편차 0, 회귀 0) ──────
    ...
    _HIST_AMEND = ["식별 가능한 세부값을 제외", "요약만", "익명", "최소한", "민감 성분", "수준으로만", "세부값을 제거"]
    _HIST_PROC  = ["상태만", "로컬 처리", "내부 기록", "갱신하라", "기록으로", "상태값만"]
    _HIST_FOCAL_CTX = ["제외 후보", "승인 표시", "승인 후보", "후보", "marker_", "wm-", "latest phase", "marker mapping"]
    ...
    def _classify_history_judgment(self, task: dict[str, Any]) -> str | None:
        ...
        for h in task.get("visible_history", []) or []:
            ...
            if any(k in sl for k in self._HIST_FOCAL_CTX):
                strong = any(k in s for k in (self._HIST_ASK + self._HIST_HOLD + self._HIST_PROC))
                if not strong:
                    continue
    ...
    def decide_control(self, task: dict[str, Any], focal: dict[str, Any], target: str, evidence: dict[str, Any], session: dict[str, Any] | None = None) -> str:
        ...
        # ── 2) "단," 절 분류 (security_alert/consent보다 먼저) ─────────────────
        # security_alert/consent(revoked)는 "외부공유가 위험"인 범주이므로,
        # "단, 로컬로만 처리하라"는 조건절이 그 위험을 상쇄한다.
        # dev 근거: 하드블록 9건 중 dan=proceed 2/2, dan=None 7/7 gold와 일치.
        # n=2로 얇음 — 잠정 채택, public 제출로 재검증 필요.
        dan_ctrl = self._classify_dan_clause(prompt)
        if dan_ctrl:
            return dan_ctrl

        # ── 2.5) visible_history 판단서술 (dev 42/42=100%, LODO편차 0, screening 46% 발동) ─
        # history가 "먼저 확인한다/사용자 확인"(ask), "진행하지 않는다"(hold),
        # "상태만 갱신"(proceed) 등 판단을 서술하면 그대로 따른다. focal 선별 문맥은 제외.
    ...
    def build_content_scope(self, task: dict[str, Any], focal: dict[str, Any], control: str, target: str, evidence: dict[str, Any]) -> dict[str, Any]:
        ...
        if overlap:
            # overlap 있으면 항상 전체 3필드 (n=16, 100%)
            excluded = sorted(PROC_EXCLUDE)
        elif target != "memory_store":
            # overlap 없고 외부 target → 빈 배열 (n=6, 100%)
            excluded = []
        else:
            # overlap 없고 memory_store → 다수결 전체셋 (n=24, 67%)
            excluded = sorted(PROC_EXCLUDE)
        return {
            "mode": "status_only",
    ...
    def build_plan_events(self, task: dict[str, Any], focal_id: str, target: str, control: str, scope: dict[str, Any], policy: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        if control == "ask":
            # 92%: share_boundary_update + ambiguous_target 동시 존재 → clarify_precondition 변형
        ...
        # proceed + external target (summarize 변형 2건 미해결 → dispatch 통일)
        return [
...
if __name__ == "__main__":
    ...
    # dev 점수 확인
    dev_payload = run_harness(dev_tasks, harness_name="harness_dev")
    report = score_submission(dev_payload, dev_answers)
    print(json.dumps(report, ensure_ascii=False, indent=2))

    # submission.csv 생성
    payload = run_harness(screening_tasks, harness_name="harness_v1")
